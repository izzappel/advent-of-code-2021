...
const main = (data) => {
  ...
  const steps = [...Array(100).keys()]
  const { numberOfFlashes } = steps.reduce(
    (acc, p) => {
      const { grid, numberOfFlashes } = step(acc.grid)
      return {
        grid,
        numberOfFlashes: acc.numberOfFlashes + numberOfFlashes,
      }
    },
    { grid, numberOfFlashes: 0 }
  )
